...
class LocalNormalizer extends INormalizer {
  ...
  _normalizeSingleWord(word, translation = '') {
    ...
    if (isVerb && isNoun && translation) {
      ...
      if (hinted === 'noun') {
        return this._nounResult(doc, word);
      }
      if (hinted === 'adj') {
  ...
  /**
   * Builds the canonical form for a noun: "a <noun>", "an <noun>" or "(unc) <noun>".
   * Countability comes from compromise's `Uncountable` tag (lexicon-based, so
   * not exhaustive — the user can still override in the dashboard).
   *
   * @param {object|null} doc  — compromise doc for `word` (null if nlp unavailable)
   * @param {string} word
   * @returns {{ canonical: string, pos: string }}
   */
  _nounResult(doc, word) {
    let singular = word;
    let uncountable = false;
    if (doc) {
      singular = doc.nouns().toSingular().out('text') || word;
      uncountable = doc.has('#Uncountable');
    }
    if (uncountable) return { canonical: `(unc) ${singular}`, pos: 'noun' };
    return { canonical: `${this._indefiniteArticle(singular)} ${singular}`, pos: 'noun' };
  }

  /**
   * Chooses "a" or "an" by the first *sound* of the word (heuristic).
   * @param {string} word
   * @returns {'a'|'an'}
   */
  _indefiniteArticle(word) {
    const w = word.trim().toLowerCase();
    // Vowel letter, but consonant sound: university, user, one, European…
    if (/^(uni(?!n)|use|usu|uti|eu|one|once|ubiq|uran|urin)/.test(w)) return 'a';
  ...
  _normalizeWithHint(word, pos) {
    switch (pos) {
      ...
      case 'adv':
        return { canonical: `(adv) ${word}`, pos: 'adv' };
      case 'noun': {
        return this._nounResult(this._nlp ? this._nlp(word) : null, word);
      }
      case 'phrase': {
 ...
 *      adv → '(adv) <word>', noun → 'a/an <singular-noun>' or '(unc) <noun>' if uncountable,

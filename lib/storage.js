...
const storage = {
  ...
  async updateWord(id, patch) {
    ...
    // Prevent accidental id overwrite (savedAt is user-editable from the dashboard)
  ...
  /**
   * Deletes multiple words by their ids in a single storage write.
   * @param {string[]} ids
   * @returns {Promise<void>}
   */
  async deleteWords(ids) {
    ...
    const result = await chromeGet(KEYS.WORDS);
    const words = (result[KEYS.WORDS] || []).filter(w => !set.has(w.id));
    await chromeSet({ [KEYS.WORDS]: words });
  },
  ...
  /**
   * Re-partitions all words that are NOT in an exported group into groups of
   * `groupSize` (oldest first). Exported groups are left untouched.
   *
   * Existing non-exported groups are reused in order (keeping their id and
   * name); extra groups are created as needed and surplus ones are removed.
   * Manual drag-and-drop assignments in those groups are not preserved.
   *
   * @param {number} groupSize
   * @returns {Promise<{ groups: number, words: number }>}
   */
  async regroupWords(groupSize) {
    const size = Math.max(1, parseInt(groupSize) || 1);
    const wordsResult  = await chromeGet(KEYS.WORDS);
    const groupsResult = await chromeGet(KEYS.GROUPS);
    const words  = wordsResult[KEYS.WORDS]   || [];
    const groups = groupsResult[KEYS.GROUPS] || [];

    const exportedIds = new Set(groups.filter(g => g.exportedAt).map(g => g.id));
    const reusable = groups
      .filter(g => !g.exportedAt)
      .sort((a, b) => a.createdAt.localeCompare(b.createdAt));

    const pool = words
      .filter(w => !exportedIds.has(w.groupId))
      .sort((a, b) => a.savedAt.localeCompare(b.savedAt));

    const usedNames = new Set(groups.map(g => g.name));
    let nameCounter = 0;
    const nextName = () => {
      do { nameCounter++; } while (usedNames.has(`Set ${nameCounter}`));
      usedNames.add(`Set ${nameCounter}`);
      return `Set ${nameCounter}`;
    };

    const keptGroups = groups.filter(g => g.exportedAt);
    const assignment = new Map(); // wordId → groupId
    let chunkCount = 0;

    for (let i = 0; i < pool.length; i += size) {
      let group = reusable[chunkCount];
      if (!group) {
        group = {
          id:         generateUUID(),
          name:       nextName(),
          createdAt:  new Date(Date.now() + chunkCount).toISOString(),
          exportedAt: null,
        };
      }
      keptGroups.push(group);
      pool.slice(i, i + size).forEach(w => assignment.set(w.id, group.id));
      chunkCount++;
    }

    const updatedWords = words.map(w => {
      if (!assignment.has(w.id)) return w;
      const keepStatus = w.status === 'raw' || w.status === 'exported';
      return { ...w, groupId: assignment.get(w.id), status: keepStatus ? w.status : 'grouped' };
    });

    await chromeSet({ [KEYS.WORDS]: updatedWords, [KEYS.GROUPS]: keptGroups });
    return { groups: chunkCount, words: pool.length };
  },

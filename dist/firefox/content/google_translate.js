...
(function () {
  ...
  // ── Cross-browser API ────────────────────────────────────────────────────
  // Resolve whichever extension API is available. In Firefox content scripts
  // the polyfill may not have run yet, so we fall back to the native globals
  // in priority order: chrome → browser → throw.
  const ext = (typeof chrome !== 'undefined' && chrome.runtime)   ? chrome
             : (typeof browser !== 'undefined' && browser.runtime) ? browser
             : null;

  if (!ext) {
    console.error('[Vocabular] No extension API found — Save buttons disabled.');
    return;
  }
